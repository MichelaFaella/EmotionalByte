...
class Transformer_Based_Model(nn.Module):
    ...
    def __init__(
        ...
        assert(
            modality in ['multi', 'text', 'audio']
        ), f'Unexpected train mode: {modality}. Expect multi, text or audio instead.'
        self._modality = modality
        self.n_classes = n_classes
        self.temp = temp        
        
        if self.modality in ['multi', 'text']:
        ...
        if self.modality in ['multi', 'audio']:
            # 1D convolution to project each modality to the shared model_dimension space
